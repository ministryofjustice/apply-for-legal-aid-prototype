if (MOJFrontend.dragAndDropSupported() && MOJFrontend.formDataSupported() && MOJFrontend.fileApiSupported()) {
  ...
  DocumentUpload.prototype.setupLabel = function () {
    const fileInputId = this.fileInput[0] ? this.fileInput[0].id : ''
    this.label = $('<label for="' + fileInputId + '" class="govuk-button govuk-button--secondary">' + this.params.dropzoneButtonText + '</label>')
